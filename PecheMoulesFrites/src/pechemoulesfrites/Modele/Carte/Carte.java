...
import pechemoulesfrites.Probabilite;
...
public class Carte {
            ...
            case Ramasser : {
                ...
                if(objet != null){
                    if((objet.getType() == Frites && !lePersonnage.hasFrites())||(objet.getType() == Moules)){ //  
                        ...
                        if(objet.getType() == Frites) Gestionnaire.getGraphe().retirerPassageFrite(objet.getCase());
                    } else {
        ...
        if (bouge) {
            ...
            if (checkColonne(x) && checkLigne(y)){
                ...
                if (getCase(x,y).getType() != Mur) {
                    ...
                    if(check){
                        if(checkColonne(xCheck) && checkLigne(yCheck)){
                            if(lePersonnage.hasFrites()){
                                ...
                                actionFinale = Attendre;
                                System.err.println("Saut impossible ("+action+") : Pas de frites");
                            }
                        } else {
                            actionFinale = Attendre;
                            System.err.println("Saut impossible ("+action+") : Mauvaises coordonnées");
                        }
                    } else {
                        movePerso(lePersonnage,getCase(x,y));
                    }
                    ...
                    actionFinale = Attendre;
                    System.err.println("Mur présent "+action+"\n");
                }
            } else {
                actionFinale = Attendre;
                System.err.println("Sortie de la carte "+action+"\n");
            }
        }
     ...
     */
    public void creerCarteDefaut(Enum_IA[] IAs){
        try {
            lireCarte(true, IAs);
        } catch (Exception ex) {
     ...
     * @param IAs les IA à donner
     */
    public void creerCartePredef(Enum_IA[] IAs){
        try {
            lireCarte(false, IAs);
    ...
    public void creerCarteAlea(Enum_IA[] IAs){
        initialiserCarte();
        int chanceMur;
        int chanceMoule;
        int chanceFrite;
        ajouterPersonnage(IAs[0],0,getHauteur()/2);
        ajouterPersonnage(IAs[1],getLargeur()/2,0);
        ajouterPersonnage(IAs[2],getLargeur()/2,getHauteur()-1);
        ajouterPersonnage(IAs[3],getLargeur()-1,getHauteur()/2);
        for(int ligne = 0;ligne<getHauteur();ligne++){
            for(int colonne = 0;colonne < getLargeur(); colonne++){
                ...
                chanceFrite = 5;
                if(colonne !=0){
                    if(cases[colonne-1][ligne].getType() == Mur){
                        chanceMur += chanceMur*0.3;
                    }else if(cases[colonne-1][ligne].getObjet() != null){
                        if(cases[colonne-1][ligne].getObjet().getType() == Moules){
                            chanceMoule =0;
                        }
                    }
                }
                if (ligne != 0){
                    if(cases[colonne][ligne-1].getType() == Mur){
                        chanceMur +=chanceMur*0.3;
                    }else if(cases[colonne][ligne-1].getObjet() != null){
                        if(cases[colonne][ligne-1].getObjet().getType() == Moules){
                            chanceMoule =0;
                        }
                    }
                }
                if(cases[colonne][ligne].getPerso().isEmpty()){
                    if(Probabilite.chance(chanceMur)){
                        cases[colonne][ligne] = new Case(colonne,ligne,Mur);
                    }
                    if(cases[colonne][ligne].getType() != Mur){
                        if(Probabilite.chance(chanceMoule)){
                            ajouterMoules(Probabilite.aleatoire(),colonne,ligne);
                        }
                        if(cases[colonne][ligne].getObjet() == null){
                            if(Probabilite.chance(chanceFrite)){
                                ajouterFrites(colonne,ligne);
                            }
                        }
                    }
                }
        ...
        Gestionnaire.addCarte(this);
    }
    public void initialiserCarte(){
        for(int ligne = 0;ligne<getHauteur();ligne++){
            for(int colonne = 0;colonne < getLargeur(); colonne++){
                cases[colonne][ligne] = new Case(colonne,ligne,Sol);
            }
      }
    }
     ...
     */
    private void lireCarte(boolean defaut, Enum_IA[] IAs) throws Exception{
        String s;
        int ia = 0;
        Scanner scan;
        if(defaut)
     ...
     */
    public void ajouterPersonnage(Enum_IA ia, int x, int y){
        Case laCase = cases[x][y];                      // on recupère la case aux coordonnées x et y
