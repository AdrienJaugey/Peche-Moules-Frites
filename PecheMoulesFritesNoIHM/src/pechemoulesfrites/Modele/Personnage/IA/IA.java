...
import static pechemoulesfrites.Modele.Carte.Graphes.PathFinder.Enum_PathFinder.*;
...
public abstract class IA {
    ...
    protected Enum_Action nextMove(){
        ...
        if (sommetPerso == sommetCible - 1) { //S'il se trouve à gauche du sommet visé
            action = AllerDroite;
        } else if (sommetPerso == sommetCible - 2) { //S'il se trouve 2 cases à gauche du sommet visé
            action = SauterDroite;
        } else if (sommetPerso == sommetCible + 1) { //Idem s'il est à droite
            action = AllerGauche;
        } else if (sommetPerso == sommetCible - 2) { //S'il se trouve 2 cases à droite du sommet visé
            action = AllerDroite;
        } else if (sommetPerso == sommetCible - Gestionnaire.getCarte().getLargeur()) { //S'il est en haut
            action = AllerBas;
        } else if (sommetPerso == sommetCible - 2*Gestionnaire.getCarte().getLargeur()) { //S'il est 2 cases en haut
            action = SauterBas;
        } else if (sommetPerso == sommetCible + Gestionnaire.getCarte().getLargeur()) { //S'il est en bas
            action = AllerHaut;
        } else if (sommetPerso == sommetCible + 2*Gestionnaire.getCarte().getLargeur()) { //S'il est 2 cases en bas
            action = SauterHaut;
        }
...
//Si on a déjà ou que l'on vient de définir une cible, on agit
    ...
    private Objet getObjetSuivant(Enum_Objet objetCible, Objet sauf){
        ArrayList<Objet> liste = new ArrayList<>();
        liste.add(sauf);
        return IA.this.getObjetSuivant(objetCible, liste);
    }
    ...
    public void setIAScore(int s) {
        this.score = s;
    }

    /**
     * Permet de retourner le score de l'IA
     * @return un entier
     */
